...
begin
  ...
  update public.payments
     set status='تم التسجيل', accounting_registered_by=auth.uid(), accounting_registered_at=now(), updated_at=now()
   where id=p_payment_id and status='منفذ' and executed_at is not null and posted_at is null
   returning * into v;
  ...
  values(auth.uid(),'PAYMENT_ACCOUNTING_REGISTERED','payment',v.id::text,
    jsonb_build_object('status','تم التسجيل','registered_at',v.accounting_registered_at));
  return v;
end $$;
...
language plpgsql security definer set search_path='public','private','pg_temp'
as $$
declare v public.payments; v_role text:=private.current_role();
begin
  ...
  if p_action='Posted' then
    update public.payments
       set status='مرحّل', posted_by=auth.uid(), posted_at=now(),
           pending_by=null,pending_at=null,pending_reason=null,pending_action=null,pending_owner_id=null,pending_follow_up_date=null,
           updated_at=now()
     where id=p_payment_id returning * into v;
    insert into public.audit_log(actor_id,action,entity_type,entity_id,details)
    values(auth.uid(),'PAYMENT_POSTED_BY_SUPERVISOR','payment',v.id::text,
      jsonb_build_object('previous_status',coalesce(v.status,'تم التسجيل'),'status','مرحّل'));
  else
    if nullif(trim(coalesce(p_reason,'')),'') is null then raise exception 'Pending justification is required'; end if;
    update public.payments
       set status='معلقة', pending_by=auth.uid(), pending_at=now(), pending_reason=trim(p_reason),
           pending_action=nullif(trim(coalesce(p_pending_action,'')),''), pending_owner_id=p_pending_owner_id,
           pending_follow_up_date=p_follow_up_date, updated_at=now()
     where id=p_payment_id returning * into v;
    insert into public.audit_log(actor_id,action,entity_type,entity_id,details)
    values(auth.uid(),'PAYMENT_MARKED_PENDING','payment',v.id::text,
      jsonb_build_object('reason',v.pending_reason,'required_action',v.pending_action,'pending_owner_id',v.pending_owner_id,'follow_up_date',v.pending_follow_up_date));
  end if;
  return v;
end $$;

-- Compatibility function retained, but GL cannot bypass Supervisor finalization anymore.
...
begin
  ...
  update public.payments set status='مرحّل',posted_by=auth.uid(),posted_at=now(),updated_at=now()
   where id=p_payment_id and status in ('تم التسجيل','معلقة') and posted_at is null returning * into v;
  if v.id is null then raise exception 'Payment must be accounting-registered first'; end if;
  insert into public.audit_log(actor_id,action,entity_type,entity_id,details)
  values(auth.uid(),'PAYMENT_POSTED_CFO_OVERRIDE','payment',v.id::text,jsonb_build_object('status','مرحّل'));
  return v;
end $$;
...
begin
  ...
  insert into public.audit_log(actor_id,action,entity_type,entity_id,details)
  values(auth.uid(),'RESERVE_DRAW_RECLASSIFIED','bank_reserve_movement',v.id::text,
    jsonb_build_object('before_requires_restoration',v_old,'after_requires_restoration',p_requires_restoration,'reason',trim(p_reason),'linked_restorations',v_restored));
  return v;
end $$;
...
with payment_uses as (
  select p.bank_account_id,p.id,p.amount,
         coalesce((select sum(r.amount) from public.bank_reserve_movements r where r.movement_type='Restoration' and r.restores_payment_id=p.id),0) restored
  from public.payments p join public.bank_accounts b on b.id=p.bank_account_id
  where p.executed_at is not null and b.account_type<>'Operational'
), manual_uses as (
  select m.bank_account_id,m.id,m.amount,m.requires_restoration,m.created_at,
         coalesce((select sum(r.amount) from public.bank_reserve_movements r where r.movement_type='Restoration' and r.restores_movement_id=m.id),0) restored
  ...
  select bank_account_id,coalesce(sum(amount),0)::numeric(18,2) executed_outflows,
         coalesce(sum(greatest(amount-restored,0)),0)::numeric(18,2) outstanding from payment_uses group by bank_account_id
), m as (
  select bank_account_id,
         coalesce(sum(amount),0)::numeric(18,2) manual_support_draws,
         coalesce(sum(greatest(amount-restored,0)) filter(where requires_restoration=true),0)::numeric(18,2) outstanding,
         max(created_at) last_movement_at
  from manual_uses group by bank_account_id
), r as (
  select bank_account_id,coalesce(sum(amount),0)::numeric(18,2) restorations,max(created_at) last_movement_at
  from public.bank_reserve_movements where movement_type='Restoration' group by bank_account_id
), lm as (
...
select bl.id,bl.company,bl.bank_name,bl.account_name,
       ...
       coalesce(p.executed_outflows,0),coalesce(m.manual_support_draws,0),coalesce(r.restorations,0),
       (coalesce(p.outstanding,0)+coalesce(m.outstanding,0))::numeric,
       ...
       nullif(greatest(coalesce(m.last_movement_at,'epoch'::timestamptz),coalesce(r.last_movement_at,'epoch'::timestamptz)),'epoch'::timestamptz),lm.movement_type
from public.bank_liquidity bl join public.bank_accounts b on b.id=bl.id
left join p on p.bank_account_id=bl.id left join m on m.bank_account_id=bl.id left join r on r.bank_account_id=bl.id left join lm on lm.bank_account_id=bl.id
